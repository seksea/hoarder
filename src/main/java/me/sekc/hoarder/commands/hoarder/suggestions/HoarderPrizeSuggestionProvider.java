...
public class HoarderPrizeSuggestionProvider implements SuggestionProvider<CommandSourceStack> {
    Hoarder plugin;

    public HoarderPrizeSuggestionProvider(Hoarder plugin) {
        this.plugin = plugin;
    }
    ...
    public CompletableFuture<Suggestions> getSuggestions(CommandContext<CommandSourceStack> context, SuggestionsBuilder builder) throws CommandSyntaxException {
		...
		for (int index=0; index < plugin.dbConn.getNumHoarderPrizes(); index++) {
			ItemStack item = plugin.dbConn.getHoarderPrizeAtIndex(index);
